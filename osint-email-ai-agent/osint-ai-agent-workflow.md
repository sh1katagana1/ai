...
def check_breaches(email):
    ...
    try:
        ...
        print(f"[HIBP STATUS] {response.status_code}")

        if response.status_code == 200:

            data = response.json()

            print(f"[HIBP BREACH COUNT] {len(data)}")
...
import os
```
This imports Python’s built-in operating system module. Purpose: to interact with environment variables and access OS-level configuration. For example:
```
os.getenv()
```
...
import requests
```
Imports the requests HTTP library. This is your API communication engine. Without this there is no GET requests, no headers and no API calls.
...
from dotenv import load_dotenv
```
Imports the .env loader. This allows the .env file, which has your environment variables, Python access instead of hardcoding secrets into scripts.
...
load_dotenv()
```
This reads the .env file and loads variables into memory. In this case the HIBP_API_KEY one. 
...
API_KEY = os.getenv("HIBP_API_KEY", "").strip()
```
1. os.getenv("HIBP_API_KEY") retrieves HIBP_API_KEY=abc123 from the environment. 
2. "", is the fallback default value. So if the value is missing use ""
3. .strip() Removes spaces, tabs and newline characters
...
def check_breaches(email):
```
Defines a reusable function. It takes in a value of an email address. 
...
```
This builds HTTP headers. Headers are metadata sent with the request. HIBP is very strict about what headers are required, in this case the user-agent and the api key.
...
```
HIBP can return full breach objects OR truncated minimal objects. We are choosing full breach objects.
...
url = f"https://haveibeenpwned.com/api/v3/breachedAccount/{email}"
```
The URL construction. Python will insert the email address where it says {email}.
...
try:
```
This is a Try block, used in Python for error handling. Examples of possible failures:
1. timeout
2. DNS failure
3. connection reset
4. SSL issue
...
Without try/except your whole agent crashes.
...
headers=headers,
```
Injects authentication and metadata.
...
params=params,
```
Automatically appends ?truncateResponse=false to the end of the URL. 
...
This forces failure after 15 seconds.
...
print(f"[HIBP STATUS] {response.status_code}")
```
Status code visibility. When it runs I will see printed to screen whether its a 200, 404, etc.
...
data = response.json()
```
Converts API JSON into Python objects.
...
print(f"[HIBP BREACH COUNT] {len(data)}")
```
Counts list length. If the email has 9 breaches, this would return 9.
...
return data
```
Sends the structured breach data back to: EmailOSINTAgent. This is your tool output.
...
```
HIBP uses 404 to mean: No breaches found
...
return []
```
This normalizes 'no breaches' into a clean predictable structure.
...
print(response.text)
```
Displays raw server response. This is a way for me to see the results of all 3 API calls normalized before they are sent to the LLM. 
...
```
This is MUCH better than silently failing. Now your orchestrator can distinguish 'No breaches' from 'Tool failure'
...
```
Catches Python/runtime failures. Examples:
...
```
Prevents crashes while preserving diagnostics.
...
This can be enhanced, but for now this is a good working start.
